...
public class BakeryFx : RecycleAdditionalFx
{
   ...
   private float HalfPathDuration => (CycleDuration - _waitInMiddlePoint) / 2;
    ...
    protected override void ProductCycle()
    {
        ...
        sequence.Append(SourceItem.DOMove(_middlePoint.position, HalfPathDuration).SetEase(_moveEase));
        ...
        sequence.Append(SourceItem.DOMove(_destinationPoint.position, HalfPathDuration).SetEase(_moveEase));
        sequence.Join(SourceItem.DOScale(0, _zoomOutTime).SetEase(_zoomOutEase).SetDelay(HalfPathDuration-_zoomOutTime));
        sequence.SetId(this);
